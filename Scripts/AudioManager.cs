...
public class AudioManager : MonoBehaviour {
    ...
    // Check if audion needs to cross fade between clips
    void Update() {
        ...
        if (cross_fade) {
            ...
            if (master_channel == "A") {
                ...
                channel_a.mute = Settings.mute;
            } else {
                channel_b.volume = 1f * bgm_volume;
                channel_b.mute = Settings.mute;
            }
        }

    }

    // Get settings volume and plays once a sound clip
    public void PlayFX(AudioClip clip) {

        aSFX.volume = Settings.volumFX;
        aSFX.mute = Settings.muteFX;
        aSFX.PlayOneShot(clip);

    }

    // Plays a new audio clip, creating its audio source.
    ...
    // Makes cross-fade or fade-in when new clip is added
    void CrossFade() {
